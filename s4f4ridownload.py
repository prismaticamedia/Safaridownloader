...
class SafariBooks:
    ...
    def __init__(self, args):
        ...
        if args.cred:
            self.display.exit("Login via credentials (--cred / --login) is permanently disabled by O'Reilly.\n"
                              "    Please login to O'Reilly in your browser and run `retrieve_cookies.py` instead.")

        if not os.path.isfile(COOKIES_FILE):
            self.display.exit("Login: unable to find `cookies.json` file.\n"
                              "    Please use the `retrieve_cookies.py` script to fetch your browser cookies.")
        ...
        if getattr(args, 'pdf', False):
            ...
            if not os.path.exists(chrome_path):
                self.display.error("Google Chrome not found at %s. Please install it to use --pdf." % chrome_path)
            else:
                pdf_file = os.path.join(self.BOOK_PATH, self.book_id + ".pdf")
                with tempfile.TemporaryDirectory() as temp_dir:
                    pdf_merger = PyPDF2.PdfMerger()
                    success = True
                    for idx, chapter in enumerate(self.book_chapters):
                        html_path = os.path.join(self.BOOK_PATH, "OEBPS", chapter["filename"])
                        chapter_pdf = os.path.join(temp_dir, f"{idx:03d}.pdf")
                        
                        cmd = [
                            chrome_path, "--headless", "--disable-gpu", 
                            "--no-pdf-header-footer", f"--print-to-pdf={chapter_pdf}", html_path
                        ]
                        res = subprocess.run(cmd, capture_output=True)
                        if res.returncode != 0 or not os.path.exists(chapter_pdf):
                            self.display.error(f"Error converting chapter {chapter['filename']} to PDF")
                            success = False
                            break
                        pdf_merger.append(chapter_pdf)
                    ...
                    if success:
                        try:
                            ...
                            self.display.error("ReportLab library not found. Clean page numbers will be skipped. Run 'pip3 install reportlab' to enable.")

                        with open(pdf_file, "wb") as output_pdf:
                            pdf_merger.write(output_pdf)
                        
                        # Add clean page numbers using ReportLab and PyPDF2
                        try:
                            ...
                            for i, page in enumerate(reader.pages):
                                ...
                                # Move to roughly the middle and near the bottom
                                can.setFont("Helvetica", 10)
                                can.drawCentredString(float(page.mediabox.width) / 2.0, 20, str(i + 1))
                                can.save()
                            ...
                            self.display.error(f"Failed to add page numbers: {e}")
        ...
        if hasattr(args, 'log') and not args.log and hasattr(self, 'display') and hasattr(self.display, 'log_file'):
            if os.path.exists(self.display.log_file):
    ...
    def get_book_info(self):
        # We now use the /api/v2/search/ endpoint to mimic v1 data
        ...
        if "results" not in response or len(response["results"]) == 0:
            self.display.exit("API: book not found in Safari Books Online.")

        res = response["results"][0]
        # Emulate v1 structure
        info = {
            "title": res.get("title", ""),
            "authors": [{"name": a} for a in res.get("authors", [])] if res.get("authors") else [],
            "identifier": self.book_id,
            "isbn": res.get("isbn", ""),
            "publishers": [{"name": p} for p in res.get("publishers", [])] if res.get("publishers") else [],
            "rights": "",
    ...
    def get_book_chapters(self, target_url=None):
        ...
        response = self.requests_provider(target_url)
        if response == 0:
        ...
        if response.get("count", 0) > sys.getrecursionlimit():
            sys.setrecursionlimit(response.get("count", sys.getrecursionlimit() + 100))
        ...
        result = []
        result.extend([c for c in formatted_results if "cover" in c["filename"].lower() or "cover" in c["title"].lower()])
        for c in result:
            formatted_results.remove(c)

        result += formatted_results
        
        next_url = response.get("next")
        return result + (self.get_book_chapters(next_url) if next_url else [])
        ...
        if link and not link.startswith("mailto"):
            if not self.url_is_absolute(link):
                if any(x in link for x in ["cover", "images", "graphics"]) or \
                    ...
                    base_chapter_url = SAFARI_BASE_URL + "/api/v2/epubs/urn:orm:book:{}/files/".format(self.book_id)
                    full_image_url = urljoin(base_chapter_url, link)
                    if full_image_url not in self.images:
    ...
    def create_toc(self):
        navmap = ""
        c = 0
        for cc in self.book_chapters:
            c += 1
            navmap += "<navPoint id=\"{0}\" playOrder=\"{1}\">" \
                      "<navLabel><text>{2}</text></navLabel>" \
                      "<content src=\"{3}\"/>" \
                      "</navPoint>\n".format(
                          "navPoint-%d" % c, c, escape(cc["title"] if cc["title"] else "Chapter %d" % c), cc["filename"]
                      )

        return self.TOC_NCX.format(
            (self.book_info["isbn"] if self.book_info.get("isbn") else self.book_id),
            1,
            escape(self.book_title),
            ", ".join(escape(aut.get("name", "")) for aut in self.book_info.get("authors", [])),
            navmap
        )
...
if __name__ == "__main__":
    ...
    arguments.add_argument(
        ...
        help="Convert the generated EPUB to PDF using Calibre's ebook-convert tool."
    )
    arguments.add_argument(
